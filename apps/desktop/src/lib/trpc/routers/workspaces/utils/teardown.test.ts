...
const echoToFile = (value: string, filePath: string) =>
...
describe("runTeardown", () => {
	...
	test("reads config from mainRepoPath and executes teardown", async () => {
		...
		const command = echoToFile("executed", markerFile);

		writeFileSync(
			join(MAIN_REPO, PROJECT_SUPERSET_DIR_NAME, "config.json"),
			JSON.stringify({ teardown: [command] }),
		);
	...
	test("uses worktreePath config when present", async () => {
		...
		writeFileSync(
			...
			JSON.stringify({ teardown: [echoToFile("executed", worktreeMarker)] }),
		);
	...
	test("prefers worktreePath config over mainRepoPath config", async () => {
		...
		writeFileSync(
			join(MAIN_REPO, PROJECT_SUPERSET_DIR_NAME, "config.json"),
			JSON.stringify({ teardown: [echoToFile("main", mainMarker)] }),
		);

		mkdirSync(join(WORKTREE, PROJECT_SUPERSET_DIR_NAME), { recursive: true });
		writeFileSync(
			join(WORKTREE, PROJECT_SUPERSET_DIR_NAME, "config.json"),
			JSON.stringify({ teardown: [echoToFile("worktree", worktreeMarker)] }),
		);
	...
	test("chains multiple teardown commands with &&", async () => {
		...
		writeFileSync(
			join(MAIN_REPO, PROJECT_SUPERSET_DIR_NAME, "config.json"),
			JSON.stringify({
				teardown: [echoToFile("created", testFile), assertFileExists(testFile)],
			}),
		);
	...
	test("reads from user override when projectId is provided", async () => {
		...
		writeFileSync(
			...
			JSON.stringify({ teardown: [echoToFile("user", userMarker)] }),
		);
